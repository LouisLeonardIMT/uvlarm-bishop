  ...
  kobuki_ros_interfaces__msg__DockInfraRed__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    kobuki_ros_interfaces__msg__DockInfraRed * data =
      (kobuki_ros_interfaces__msg__DockInfraRed *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!kobuki_ros_interfaces__msg__DockInfraRed__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          kobuki_ros_interfaces__msg__DockInfraRed__fini(&data[i]);
        }
        free(data);
        return false;
      }

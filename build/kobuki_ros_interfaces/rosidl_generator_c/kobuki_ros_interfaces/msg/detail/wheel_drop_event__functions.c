  ...
  kobuki_ros_interfaces__msg__WheelDropEvent__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    kobuki_ros_interfaces__msg__WheelDropEvent * data =
      (kobuki_ros_interfaces__msg__WheelDropEvent *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!kobuki_ros_interfaces__msg__WheelDropEvent__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          kobuki_ros_interfaces__msg__WheelDropEvent__fini(&data[i]);
        }
        free(data);
        return false;
      }

  ...
  kobuki_ros_interfaces__msg__SensorState__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    kobuki_ros_interfaces__msg__SensorState * data =
      (kobuki_ros_interfaces__msg__SensorState *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!kobuki_ros_interfaces__msg__SensorState__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          kobuki_ros_interfaces__msg__SensorState__fini(&data[i]);
        }
        free(data);
        return false;
      }
